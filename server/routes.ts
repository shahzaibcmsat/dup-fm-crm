...
import { sendEmail, isGmailConfigured, getAuthorizationUrl, exchangeCodeForTokens } from "./gmail";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/emails/generate-reply", async (req, res) => {
    try {
      ...
      const emailsResult = await storage.getEmailsByLeadId(leadId);
    ...
    try {
      ...
      if (!authUrl) {
        return res.status(500).json({ 
          message: "Gmail not configured. Please set GMAIL_CLIENT_ID and GMAIL_CLIENT_SECRET" 
        });
      }
  ...
  app.get("/api/auth/callback", async (req, res) => {
    try {
      ...
      // Display refresh token for user to save
        ...
        <html>
          <head>
            <style>
              ...
              code { background: #1f2937; color: #10b981; padding: 8px 12px; border-radius: 4px; display: block; word-break: break-all; }
          ...
          <body>
            ...
            ${refreshToken ? `
              ...
              <div class="warning">
                <strong>⚠️ Warning:</strong> Keep this token secure! Anyone with this token can send emails from your account.
              </div>
            ` : '<p>Tokens received. Please check server console for details.</p>'}
  ...
  app.get("/api/debug/test-mailbox", async (req, res) => {
    try {
      ...
      // Try to fetch emails to test access
      const emails = await fetchNewEmails();
      
      res.json({ 
        success: true, 
        message: "Gmail is accessible",
      ...
      res.json({ 
        success: false, 
        error: error.message,
        suggestion: 'Check if GMAIL_REFRESH_TOKEN is valid and not expired'
      });
    }
  });
  ...
  app.get("/api/leads", async (req, res) => {
    try {
      // Check if client wants pagination (has limit or cursor param)
      const wantsPagination = req.query.limit || req.query.cursor;
      
      if (wantsPagination) {
        const limit = req.query.limit ? parseInt(req.query.limit as string) : 100;
        const cursor = req.query.cursor as string | undefined;
        const result = await storage.getAllLeads(limit, cursor);
        res.json(result);
      } else {
        // Backward compatibility: return simple array with reasonable limit
        const result = await storage.getAllLeads(1000);
  ...
  app.post("/api/leads/:id/send-email", async (req, res) => {
    try {
      ...
      const existingEmailsResult = await storage.getEmailsByLeadId(lead.id, 100); // Get last 100 emails
      const lastEmail = existingEmailsResult.emails.length > 0 
        ? existingEmailsResult.emails[0] // Already sorted by sentAt desc
        : null;
      ...
      // Use threadId as conversationId for consistency
      if (lastEmail?.conversationId && !result.threadId) {
        result.threadId = lastEmail.conversationId;
      }
      ...
      const emailData = insertEmailSchema.parse({
        ...
        conversationId: result.threadId || null,
  ...
  app.get("/api/emails/:leadId", async (req, res) => {
    try {
      ...
      if (wantsPagination) {
        ...
        const cursor = req.query.cursor as string | undefined;
        const result = await storage.getEmailsByLeadId(req.params.leadId, limit, cursor);
        res.json(result);
      } else {
        // Backward compatibility: return simple array with all emails (limited to 500)
        const result = await storage.getEmailsByLeadId(req.params.leadId, 500);
  ...
  app.post("/api/emails/sync", async (req, res) => {
    try {
      const { fetchNewEmails, getInReplyToHeader } = await import("./gmail");
  ...
  app.post("/api/import/file", upload.single("file"), async (req, res) => {
    try {
      ...
      const existingLeadsResult = await storage.getAllLeads(10000); // High limit for import
      const existingEmails = new Set(existingLeadsResult.leads.map(lead => lead.email.toLowerCase()));
  ...
  app.get("/api/companies", async (req, res) => {
    try {
      ...
      const companies = await storage.getAllCompanies(limit);
      res.json(companies);
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  ...
  app.get("/api/inventory", async (req, res) => {
    try {
      ...
      if (wantsPagination) {
        const limit = req.query.limit ? parseInt(req.query.limit as string) : 100;
        const cursor = req.query.cursor as string | undefined;
        const result = await storage.getAllInventory(limit, cursor);
        res.json(result);
      } else {
        // Backward compatibility: return simple array with reasonable limit
        const result = await storage.getAllInventory(1000);
